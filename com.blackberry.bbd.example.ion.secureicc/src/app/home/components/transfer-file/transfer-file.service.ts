...
export class TransferFileService {
  ...
  sendFileToApp(filePath, applicationId) {
    return new Promise((resolve, reject) => {
      window.plugins.GDAppKineticsPlugin.sendFileToApp(filePath, applicationId, result => {
        resolve(result);
      }, error => {
        reject(error);
      });
  ...
  retrieveFiles() {
    return new Promise((resolve, reject) => {
      window.plugins.GDAppKineticsPlugin.retrieveFiles(result => {
        resolve(result);
      }, error => {
        reject(error);
      });
